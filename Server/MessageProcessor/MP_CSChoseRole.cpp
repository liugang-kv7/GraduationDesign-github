...
class MP_CSChoseRole : public MessageProcessorBase
{
public:
	virtual int32_t ProcessRequest(int32_t user_id, const CMessage& mess)
	{
		...
		CSChoseRole* oldproto = (CSChoseRole*)mess.m_proto;
		SCChoseRole* proto = new SCChoseRole;

		proto->set_role_id(oldproto->role_id());
		proto->set_player_id(playerid);

		CMessage newmess;
		newmess.m_head.m_framenum = room->m_syn->GetNowFrame();
		newmess.m_head.m_message_id = MSG_SCChoseRole;
		newmess.m_proto = proto;

		room->m_syn->Add(newmess);
		return 1;
	}
}MP_CSChoseRole_instance;

REGISTE_MESSAGE_ID(MSG_CSChoseRole, CSChoseRole, &MP_CSChoseRole_instance);
